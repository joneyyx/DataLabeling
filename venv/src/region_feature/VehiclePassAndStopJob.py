...
def main():
    data = spark.sql("select engine_serial_number, occurrence_date_time, wheel_based_vehicle_speed, gps_vehicle_speed, province, city, high_resolution_total_vehicle_distance, latitude, longitude from data_label.tel_hb_labeled_sample where report_date >= '20200701' and report_date <= '20200731'")
    ...
    filledNullCityDF_BACK = fillNullWithBackward(filledNullCityDF, ["engine_serial_number"], "occurrence_date_time", "city", "city")
    filledNullProvinceDF = fillNullWithForward(filledNullCityDF_BACK, ["engine_serial_number"], "occurrence_date_time", "province", "province")
    filledNullProvinceDF_BACK = fillNullWithBackward(filledNullProvinceDF, ["engine_serial_number"], "occurrence_date_time", "province", "province")
    ...
    cleanDF = filledNullProvinceDF_BACK.withColumn("vehicle_speed", coalesce(filledNullProvinceDF_BACK["wheel_based_vehicle_speed"], filledNullProvinceDF_BACK["gps_vehicle_speed"])).\
      ...
      withColumn("TEL_CALC_DATE", next_day(col("Date_Time"), 'Sun')) #calculate date is the last day of week
    ...
    searchParkPointDF = aggDF.withColumn("delta_time", (unix_timestamp("max_time")-unix_timestamp("min_time"))/60). \
        withColumn("TEL_CALC_DATE", next_day(col("Date_Time"), 'Sun')).\
        filter(col("delta_time")>=30)



    # 周滑动窗口-停靠频次(in order)
    aggDF2 = searchParkPointDF.groupBy("engine_serial_number", "city", "TEL_CALC_DATE").count().withColumnRenamed("count", "HB_CMP_1WEEK_STOP_CITY_CNT")
    parkMapDF = aggDF2.groupBy("engine_serial_number", "TEL_CALC_DATE").agg(collect_list(create_map(col("city"), col("HB_CMP_1WEEK_STOP_CITY_CNT"))).alias("city_park_map"))



    vehicleParkDF = searchParkPointDF.groupBy("engine_serial_number", "TEL_CALC_DATE").agg(collect_set("city").alias("city_park_set")).\
        withColumn("city_park_set", sort_array(col("city_park_set")))

    cityParkByWeekDF = vehicleParkDF.join(parkMapDF, on=["engine_serial_number", "TEL_CALC_DATE"])
    ...
    # Window
    wSpec = Window.partitionBy("engine_serial_number", "TEL_CALC_DATE").orderBy("occurrence_date_time")
    windowDF = cleanDF.select("engine_serial_number", "occurrence_date_time", "city", "TEL_CALC_DATE").withColumn("city_lag", lead("city").over(wSpec)).\
        withColumn("label_pass_city", when(col("city")==col("city_lag"), lit(0)).otherwise(lit(1))). \
        filter(col("city_lag").isNotNull())

    aggDF3 = windowDF.groupBy("engine_serial_number", "city", "TEL_CALC_DATE").agg(sum("label_pass_city").alias("HB_CMP_1WEEK_PASS_CITY_CNT"))
    ...
    joinDF = aggDF2.join(aggDF3, on = ["engine_serial_number", "city", "TEL_CALC_DATE"], how="outer")
...
def fillNullWithBackward(df, partition_columns_list, order_columns, fill_column, target_column):
  ...
  df_new = df.withColumn(target_column, first(fill_column, True).over(wspec)) #True: fill with last not-null
  return df_new

test = fillNullWithBackward(data, ["engine_serial_number"], "occurrence_date_time", "city", "city")
