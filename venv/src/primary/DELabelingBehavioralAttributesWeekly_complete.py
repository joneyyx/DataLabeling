...
def main(dt):
    ...
    resultJoinDF = hbResultDF.join(efpaResultDF, on= ["esn", "calc_date_sunday"], how= "outer").\
    ...
    fcOriginDF = spark.table("qa_prd_primary.tel_fault_code_detail").select("ENGINE_SERIAL_NUMBER", "OCCURRENCE_DATE_TIME", "FAULT_CODE").\
        withColumn("report_date", date_format(to_date("OCCURRENCE_DATE_TIME"), "yyyyMMdd")). \
        withColumn("calc_date", to_date("OCCURRENCE_DATE_TIME")).\
        filter((col("report_date") <= dt) & (col("report_date") >= monday)). \
        withColumn("calc_date_sunday", next_day(col("calc_date"), "Sun")).\
        groupBy("ENGINE_SERIAL_NUMBER", "calc_date_sunday", "FAULT_CODE").agg(count("ENGINE_SERIAL_NUMBER").alias("count_nbr")).cache()
    ...
    fcProcessedDF = fcOriginDF.withColumn("struct_type", struct("FAULT_CODE","count_nbr")).\
        ...
        groupBy("ENGINE_SERIAL_NUMBER", "calc_date_sunday").agg(collect_list(col("struct_type")).alias("key_value")).\
        withColumn("tel_cmp_1week_fc_cnt", array_map(col("key_value"))).\
        drop("key_value").\
        withColumnRenamed("ENGINE_SERIAL_NUMBER", "esn")

    bulletin_Hb_Efpa_Fc  = bulletin_Hb_Efpa.join(fcProcessedDF, on=["esn", "calc_date_sunday"], how="left")
    ...
    claimProcessedDF = claimOriginDF.groupBy("esn", "calc_date_sunday").agg(count("CLAIM_NUMBER").alias("tel_cmp_1week_claim_cnt"))

    bulletin_Hb_Efpa_Fc_Claim = bulletin_Hb_Efpa_Fc.join(claimProcessedDF, on=["esn", "calc_date_sunday"], how="left")

    saveAsTable(bulletin_Hb_Efpa_Fc_Claim, "de_labeling", "bulletin_Hb_Efpa_Fc_Claim", hivePath+"bulletin_Hb_Efpa_Fc_Claim/", "overwrite")
...
def weeklyHBRun(dt, hivePath):
    ...
    hbResultDF = hbAggedDF.select("esn",
        "calc_date_sunday",
        "1week_last_operation_days",
        "tel_cmp_1week_6oclock_engruntime_h",
        "tel_cmp_1week_12oclock_engruntime_h",
        "tel_cmp_1week_18oclock_engruntime_h",
        "tel_cmp_1week_24oclock_engruntime_h",
        "hb_cmp_1week_average_speed_km_h",
        "hb_cmp_1week_average_speed_hbcount",
        "hb_cmp_1week_idleincluded_average_speed_km_h",
        "hb_cmp_1week_idleincluded_average_speed_hbcount",
        "hb_cmp_1week_plain_count",
        "hb_cmp_1week_hills_count",
        "hb_cmp_1week_mountains_count",
        "hb_cmp_1week_plateau_count",
        "hb_cmp_1week_highplateau_count",
        "hb_cmp_1week_landscape_hbcount")
...
def weeklyEFPARun(dt):
    ...
    efpaAggedDF  = efpaProcessedDF.groupBy("esn", "calc_date_sunday").\
        agg(sum("TEMP_tel_cmp_1week_windresistancework_kwh").alias("tel_cmp_1week_windresistancework_kwh"),
            ...
            sum(col("VSPDave") * col("drivetime") / 3600).alias("efpa_cmp_1week_mileage_km"),   #如果hb算法实现，该运行里程算法就不用EFPA计算
            sum("fuelvolumetotal").alias("efpa_cmp_1week_fuelvolumetotal_l"),                   #如果hb算法实现，该运行油耗就不用EFPA计算
    ...
    efpaResultDF = efpaProcessAggedDF.select("esn",
        ...
        "efpa_cmp_1week_fuelvolumetotal_l", #如果hb算法实现，该运行油耗就不用EFPA计算
        "efpa_cmp_1week_mileage_km")        #如果hb算法实现，该运行里程算法就不用EFPA计算
